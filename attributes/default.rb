...
default['zookeeperd']['server_packages'] = %w(zookeeper-server zookeeper)
...
if node.platform_family?('rhel', 'fedora', 'suse')
  ...
  default['zookeeperd']['cloudera']['baseurl'] = "http://archive.cloudera.com/cdh5/redhat/#{node['zookeeperd']['install']['platform_version'].to_i}/#{node['kernel']['machine']}/cdh/5/"
  default['zookeeperd']['cloudera']['gpgkey'] = "http://archive.cloudera.com/cdh5/redhat/#{node['zookeeperd']['install']['platform_version'].to_i}/#{node['kernel']['machine']}/cdh/RPM-GPG-KEY-cloudera"
elsif node.platform_family?('debian')
  default['zookeeperd']['cloudera_repo'] = true
  default['zookeeperd']['server_packages'] = %w(zookeeperd zookeeper) unless node['zookeeperd']['cloudera_repo']
  default['zookeeperd']['service_name'] = 'zookeeper' unless node['zookeeperd']['cloudera_repo'] || node['zookeeperd']['install']['method'] == 'jar'
  default['zookeeperd']['cloudera']['uri'] = "http://archive.cloudera.com/cdh5/#{node['platform']}/#{node['lsb']['codename']}/amd64/cdh"
...
default['java']['jdk_version'] = '7' unless node['platform'] == 'centos' && node['platform_version'] < '7'
