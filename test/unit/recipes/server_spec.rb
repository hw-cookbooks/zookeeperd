...
describe 'zookeeperd::server' do
  ...
  before do
    node.normal['zookeeperd']['cluster'][:auto_discovery] = false
    node.normal['zookeeperd'][:zk_id] = 99
  end
  ...
  shared_examples 'installing a server' do
    ...
    it 'installs custom packages if set' do
      node.normal['zookeeperd'][:server_packages] = %w(foo bar)
  ...
  shared_examples 'using cloudera repo' do
    before { node.normal['zookeeperd'][:cloudera_repo] = true }
  ...
  shared_examples 'using runit' do
    before { node.normal['zookeeperd'][:init] = 'runit' }
