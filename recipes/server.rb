...
unless node['zookeeperd']['zk_id']
  ...
  if node['zookeeperd']['auto_id'].to_s != 'rand'
    node.normal['zookeeperd'][:zk_id] = `hostid`.to_i(16)
  end
  if node['zookeeperd']['zk_id'].nil? || node['zookeeperd']['zk_id'] > max_uint
    node.normal['zookeeperd'][:zk_id] = rand(max_uint)
  end
end
...
unless node['zookeeperd']['zk_id']
  ...
  node.normal['zookeeperd'][:zk_id] = cur_id + 1
  node.save
end
...
ruby_block 'mark as a zookeeper node' do
  block { node.normal[:zookeeperd_server] = true }
end
